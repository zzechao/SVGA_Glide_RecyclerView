    ...
    fun decodeBitmapFrom(data: T, reqWidth: Int, reqHeight: Int): Bitmap? {
        return BitmapFactory.Options().run {
            ...
            if (userInBitmap) {
                ...
                try {
                    // 数据加载
                    LogUtils.debug(TAG, "onDecode use inBitmap expectedWidth:$expectedWidth expectedHeight:$expectedHeight")
                    ...
                    if (this.inBitmap != null) {
                        try {
                            ...
                            this.inBitmap = null
                            // 再次调用进行加载
                            LogUtils.debug(TAG, "onDecode not use inBitmap")
                            result = onDecode(data, this)
                        } catch (resetException: IOException) {
                            LogUtils.error(TAG, "onDecode error", resetException)
                        }
                    }
                } finally {
                ...
                LogUtils.debug(TAG, "onDecode common")
                onDecode(data, this)
            }
        }
    }
